...
///
/// For DNA alphabet {$=0, A=1, C=2, G=3, T=4}:
/// C[0] = 0
/// C[1] = count($)
/// C[2] = count($) + count(A)
...
mod tests {
    ...
    fn test_c_array_basic() {
        // text: A C G T $  => freq: $=1, A=1, C=1, G=1, T=1
        let text = vec![A, C, G, T, SENTINEL];
        let c = CArray::from_text(&text);
        assert_eq!(c.data, [0, 1, 2, 3, 4]);
    }

    #[test]
    fn test_c_array_repeated() {
        // text: A A C C $  => freq: $=1, A=2, C=2, G=0, T=0
        let text = vec![A, A, C, C, SENTINEL];
        let c = CArray::from_text(&text);
        assert_eq!(c.data, [0, 1, 3, 5, 5]);
    }
